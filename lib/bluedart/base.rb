...
module Bluedart
  class Base
    ...
    def namespaces
      ...
      ns[:array]   = {key:'ns6', value: 'http://schemas.microsoft.com/2003/10/Serialization/Arrays'}
      ns
    end
    ...
    #
    # Returns Hash
    def namespace_hash(message)
      opt = {}
      namespaces.each do |type, attrs|
        case message
        when "RegisterPickup"
          next if type == :shipment
        when "GenerateWayBill"
          next if (type == :pickup || type == :array)
        end
        key = "xmlns:#{attrs[:key]}"
        opt[key] = attrs[:value]
      end
    ...
    def awb_xml(xml, value)
      ns_key = "#{namespace_key(:array)}"
      xml[ns_key].string value
      xml
    end
    ...
    def xml_key_value(key, value, xml)
      ...
      elsif value.is_a?(Array) && key == 'ns5:AWBNo' # Horribly monkey patched. TODO: Fix
        xml.send(key) do |xml|
          xml = awb_xml(xml, value.first)
        end
    ...
    def request_xml(opts)
      ...
      builder = Nokogiri::XML::Builder.new(encoding: 'UTF-8') do |xml|
        xml[envelope_ns_key].Envelope(namespace_hash(opts[:message])) {
